...
-( void ) receive:(NSArray *)data
{
    ...
    for( int i = 0; i < count; i++ )
    {
        ...
        NSString * userAddress = [Twiiterer getUserLocation:o ];
        

        if( [ self getUserAtName: userName ] == nil )
        {
            Person * user = [ [ Person alloc] initWithName:userName screenName: userScreenName photo:userImageUrl address:userAddress ];
