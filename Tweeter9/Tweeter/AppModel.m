...
-( void ) receive:(NSArray *)data
{
    ...
    for( int i = 0; i < count; i++ )
    {
        ...
        //NSLog(@"%@ -> %@", [ Twiiterer getUserName:o ], [ Twiiterer getUserLocation:o ]);
        
        NSString * userName = [ Twiiterer getUserName:o ];
        NSString * userAdderss = [ Twiiterer getUserLocation:o ];
        ...
        {
            Person * user = [ [ Person alloc] initWithName:userName screenName: userScreenName photo:userImageUrl address:userAdderss ];
...
{
    [ self->engine sendTweet:messsage ];
}
