...
-(void)ccPhysicsCollisionPostSolve:(CCPhysicsCollisionPair *)pair penguin:(CCNode *)nodeA wildcard:(CCNode *)nodeB
{
    if(_currentPenguin.launched)
    {
        ...
        // if energy is large enough, remove the penguin
        if (energy > 70000.f)
        {
            [self penguinRemoved:nodeA];
        }
    }
}
...
-(void) penguinRemoved: (CCNode *)penguin {
    // load particle effect
    CCParticleSystem *explosion = (CCParticleSystem *)[CCBReader load:@"ExplosionPenguin"];
    // make the particle effect clean itself up, once it is completed
    explosion.autoRemoveOnFinish = TRUE;
    // place the particle effect on the seals position
